...
const sameJoinLetters = (partOne, partTwo) => partOne[partOne.length-1] === partTwo[0];

const getNameEnd = (gender) => gender ? listRand(namesInfo[gender]) : listRand([...namesInfo['female'], ...namesInfo['male']]);
...
const getElfName = (length) => {
	...
	const translatedNameParts = sortedNameParts.map(value=> namesInfo.sindarin[value]);
...
function getName(gender, race){
	...
	const last = race === 'elf' ? getElfName(13) : randomDraw(namesInfo.last, 2).sort(sortNameElements).join("");
